<template>
    <template v-if="category">
        ...
        <tr class="hover:bg-gray-50 transition-all duration-200 ease-out group">
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 cursor-pointer flex items-center mx-2"
                @click="toggle" :style="{ paddingLeft: `${level * 24 + (hasChildren ? 0 : 24)}px` }">
                <span v-if="hasChildren"
                    ...
                    :class="{ 'transform rotate-90 text-blue-500': isOpen, 'text-gray-400': !isOpen }">
                    <i class="fas fa-chevron-right text-xs"></i>
                </span>
                <span class="flex items-center">
                    <span class="w-2 h-2 rounded-full mr-3 transition-all duration-200" :class="{
                        'bg-blue-500': level === 0,
                        'bg-green-500': level === 1,
                        'bg-purple-500': level === 2,
                        'bg-yellow-500': level >= 3,
                    }"></span>
                    <span class="transition-all duration-200" :class="{ 'font-semibold text-gray-900': level === 0 }">
                        {{ category.name }}
                    </span>
                </span>
            </td>

            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 transition-all duration-200">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="relative inline-block text-left">
                    <button
                        ...
                        @click.stop="toggleDropdown"> <i class="fas fa-ellipsis-v"></i> </button>
        ...
        <transition @before-enter="beforeEnter" @enter="enter" @leave="leave" :css="false">
            <div v-if="hasChildren && isOpen" class="overflow-hidden">
                <CategoryItem v-for="(child, index) in category.all_children" :key="index" :category="child"
                    :level="level + 1" />
            </div>
        </transition>
    </template>
...
const hasChildren = computed(() => props.category.all_children && props.category.all_children.length > 0)
