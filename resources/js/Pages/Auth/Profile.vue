<template>
  ...
  <div class="bg-gradient-to-r from-indigo-50 to-white p-6 border-b border-indigo-100 flex justify-between items-center">
    <div>
        ...
        {{ $page.props.auth.user?.email || 'Chưa cập nhật' }}
          ...
          <section id="work-info" class="bg-white rounded-xl shadow-md overflow-hidden">
            <div class="bg-gradient-to-r from-indigo-50 to-white p-6 border-b border-indigo-100 flex justify-between items-center">
              ...
              <button class="px-4 py-2 bg-white border border-gray-200 text-gray-500 rounded-lg cursor-not-allowed" disabled>
                <i class="fas fa-pen mr-2"></i>Chỉnh sửa
              </button>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-1">
                ...
                <p class="text-gray-800 font-medium">{{ $page.props.auth.user?.employee_code }}</p>
              </div>
              <div class="space-y-1">
                <label class="block text-sm font-medium text-gray-500">Vai trò</label>
                <p class="text-gray-800">{{ $page.props.auth.user?.role }}</p>
              </div>
              <div class="space-y-1">
                <label class="block text-sm font-medium text-gray-500">Vị trí công việc</label>
                <p class="text-gray-800">{{ $page.props.auth.user?.position }}</p>
              </div>
              <div class="space-y-1">
              ...
              <div class="space-y-1">
                ...
                <p class="text-gray-800">{{ $page.props.auth.user?.start_date }}</p>
              </div>
            </div>
          </section>
          ...
          <section id="system-info" class="bg-white rounded-xl shadow-md overflow-hidden">
            ...
            <div class="p-6 space-y-6">
              ...
              <div v-for="activity in $page.props.auth.user?.activities" :key="activity.id" class="border-l-2 border-indigo-200 pl-4 py-1">
                <p class="text-sm">{{ activity.action }} - <span class="text-gray-500 text-xs">{{ activity.time }}</span></p>
              </div>
              <div class="grid grid-cols-2 gap-2 mt-2">
                <div v-for="perm in $page.props.auth.user?.permissions" :key="perm" class="flex items-center gap-1 bg-indigo-50 p-2 rounded">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
          ...
          <form @submit.prevent="changePassword" class="space-y-4">
            ...
            <div class="flex justify-end gap-2">
              <button type="button" @click="showPasswordModal = false" class="px-4 py-2 bg-gray-200 rounded">Hủy</button>
...
<style scoped>
.nav-item.active { background-color: #e0e7ff; }
</style>
