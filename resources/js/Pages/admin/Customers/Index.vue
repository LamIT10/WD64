        ...
        <div class="bg-gradient-to-b from-gray-100 to-gray-50 p-6 min-h-screen">
            ...
            <div class="p-4 shadow-sm rounded-lg bg-white mb-4 flex justify-between items-start border border-gray-200">
                ...
                <div class="flex items-center space-x-3">
                    <!-- Search bar -->
                    <div class="relative">
                        <input type="text" v-model="search" @keydown.enter="handleSearch" placeholder="Tìm theo tên, số điện thoại, email..." class="w-64 pl-10 pr-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:outline-none focus:ring-indigo-500 focus:border-transparent transition-all" />
                    ...
                    <div ref="dropdownRef" class="relative">
                        <button @click="toggleDropdown" class="px-4 py-2 border border-indigo-200 bg-indigo-50 text-indigo-700 rounded-lg text-sm font-medium hover:bg-indigo-100 hover:border-indigo-300 transition-all duration-200 flex items-center gap-2">
                            <i class="fas fa-columns"></i> Cột hiển thị
                        </button>
                        <div v-if="showColumnDropdown" class="absolute right-0 z-20 mt-2 w-80 bg-white border border-indigo-100 rounded-xl shadow-lg p-4">
                            <div class="grid grid-cols-2 gap-x-6 gap-y-3">
                                <label v-for="field in columnOptions" :key="field" class="flex items-center space-x-2 text-sm text-gray-800">
                                    <input type="checkbox" :value="field" v-model="visibleColumns" class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500" />
            ...
            <div class="mb-4 border-b border-gray-200">
                <div class="flex items-center">
                    <ul class="flex flex-wrap -mb-px">
                        <li class="mr-2">
                            <button @click="activeTab = 'active'" :class="{ 'text-indigo-600 border-indigo-600': activeTab === 'active', 'text-gray-500 hover:text-gray-600 hover:border-gray-300': activeTab !== 'active' }" class="inline-block p-3 border-b-2 rounded-t-lg text-sm font-medium">
                                Khách hàng đang hoạt động
                            </button>
                        </li>
                        <li class="mr-2">
                            <button @click="activeTab = 'inactive'" :class="{ 'text-indigo-600 border-indigo-600': activeTab === 'inactive', 'text-gray-500 hover:text-gray-600 hover:border-gray-300': activeTab !== 'inactive' }" class="inline-block p-3 border-b-2 rounded-t-lg text-sm font-medium">
                    ...
                    <div class="relative ml-2" v-if="selectedCustomers.length > 0" ref="actionDropdownRef">
                        <button @click="toggleActionDropdown" class="px-3 py-1.5 border border-indigo-200 bg-indigo-50 text-indigo-700 rounded-lg text-xs font-medium hover:bg-indigo-100 hover:border-indigo-300 transition-all duration-200 flex items-center gap-1.5">
                            <i class="fa fa-bars icon-btn"></i> Thao tác
                        </button>
                        <div v-if="showActionDropdown" class="absolute right-0 z-20 mt-1 w-40 bg-white border border-gray-200 rounded-lg shadow-md py-1">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden animate-fade-in">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="text-xs text-gray-700 bg-indigo-50 border-b border-indigo-300">
                            <tr>
                                ...
                                <th v-if="visibleColumns.includes('name')" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">
                                    <div class="flex items-center justify-center space-x-1">
                                        <span>Tên</span>
                                    </div>
                                </th>
                                <th v-if="visibleColumns.includes('phone')" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">
                                    Số điện thoại
                                </th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="customer in props.customers.data" :key="customer.id" @click="handleClick(customer.id)" class="hover:bg-gray-50 cursor-pointer transition-colors duration-150">
                                <td class="px-4 py-4 whitespace-nowrap" @click.stop>
                                    <input type="checkbox" :value="customer.id" v-model="selectedCustomers" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
                                </td>
                                <td v-if="visibleColumns.includes('name')" class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <img :src="customer.avatar ? `/storage/${customer.avatar}` : '/images/default-ava.png'" alt="Avatar" class="h-10 w-10 object-cover shadow-sm rounded-md" />
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ customer.name }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td v-if="visibleColumns.includes('phone')" class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">{{ customer.phone || '-' }}</td>
                                <td v-if="visibleColumns.includes('email')" class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ customer.email || '-' }}</td>
                                <td v-if="visibleColumns.includes('rank')" class="px-6 py-4 whitespace-nowrap text-center">
                ...
                <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between bg-gray-50/50">
                    <div class="text-sm text-gray-600 font-medium">
                        Hiển thị <span class="font-semibold">{{ props.customers.from || 1 }}</span> đến <span class="font-semibold">{{ props.customers.to || 0 }}</span> của <span class="font-semibold">{{ props.customers.total || 0 }}</span> kết quả
                    </div>
                    <div class="flex items-center space-x-2">
                        <Link v-if="props.customers.prev_page_url" :href="addStatusToUrl(props.customers.prev_page_url)" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-all duration-200">
                        ...
                        <Link v-if="props.customers.next_page_url" :href="addStatusToUrl(props.customers.next_page_url)" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 transition-all duration-200">
