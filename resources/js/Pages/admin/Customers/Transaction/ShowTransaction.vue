      ...
      <div class="bg-indigo-50 rounded-lg p-6 mb-6 border border-indigo-100">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
          ...
          <div class="flex items-center space-x-2">
            ...
            <Waiting v-if="customerId" :route-name="'admin.customers.debt-orders'"
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="(item, index) in sortedHistory" :key="index" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  {{ item.creator?.name ?? 'Không xác định' }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-gray-900">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div v-if="item.proof_image">
                    <a :href="`/storage/${item.proof_image}`" target="_blank">
                      <img :src="`/storage/${item.proof_image}`" alt="Ảnh giao dịch"
                        class="w-16 h-16 object-cover rounded shadow" />
                    </a>
                  </div>
                  <span v-else class="text-sm text-gray-400 italic">Không có</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-gray-900">
