<template>
  <AppLayout>
    <div class="bg-white rounded-lg shadow p-6 w-full mx-auto m-2 pb-[80px]">
      <ExportMonthComparisonCard 
        :data="exportOrderMonthComparison" 
      />
...
import ExportMonthComparisonCard from './components/ExportMonthComparisonCard.vue'
...
const props = defineProps({
  ...
  exportOrderMonthComparison: Object
})
...
const handleMonthFilter = (month) => {
  router.get(route('admin.reports.export'), { month }, {
    preserveScroll: true,
    preserveState: true,
    only: ['exportStatusSummary', 'selectedMonth'],
    onSuccess: () => {
