<template>
    <AppLayout>
        <div class="bg-gradient-to-b from-gray-100 to-gray-50 p-6 min-h-screen">
            ...
            <div class="p-4 shadow-sm rounded-lg bg-white mb-4 flex justify-between items-start border border-gray-200">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <Waiting route-name="admin.users.create" v-can="'admin.user.create'" :route-params="{}">
        ...
        <div v-if="showConfirmModal"
            class="fixed top-6 left-1/2 transform -translate-x-1/2 z-[9999] w-full max-w-md px-4">
            <div class="bg-white rounded-xl shadow-2xl border border-gray-200 animate-slide-down">
                <!-- Header -->
                <div class="flex justify-between items-center p-4 border-b border-gray-100">
                    <h3 class="text-base font-semibold text-gray-800">{{ confirmModalTitle }}</h3>
                    <button @click="cancelConfirm" class="text-gray-400 hover:text-gray-600">
                        <i class="fas fa-times text-sm"></i>
                    </button>
                </div>
                <!-- Body -->
                <div class="p-4 text-sm text-gray-700 whitespace-normal">
                    {{ confirmModalMessage }}
                </div>
                <!-- Footer -->
                <div class="flex justify-end space-x-2 p-4 border-t border-gray-100">
                    <button @click="cancelConfirm"
                        class="px-4 py-1.5 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 text-sm"
                        :disabled="isProcessingConfirm">
                        Hủy
                    </button>
                    <button @click="confirmBulkUpdate"
                        class="px-4 py-1.5 bg-indigo-600 text-white rounded hover:bg-indigo-700 text-sm"
                        :disabled="isProcessingConfirm">
                        <span v-if="isProcessingConfirm">Đang xử lý...</span>
...
const bulkUpdateStatus = (newStatus) => {
    confirmModalTitle.value = 'Xác nhận thay đổi trạng thái';
    confirmModalMessage.value = `Bạn có chắc muốn chuyển ${selectedUsers.value.length} nhân viên sang trạng thái ${newStatus === 'active' ? 'Đang làm việc' : 'Ngừng làm việc'}?`;
    pendingStatusChange = newStatus;
    showConfirmModal.value = true;
}
const cancelConfirm = () => {
    if (!isProcessingConfirm.value) {
