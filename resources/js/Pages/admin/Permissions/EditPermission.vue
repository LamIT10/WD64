                    ...
                    <div class="w-[100%] mx-auto my-[10px] gap-x-[20px]">
                        <form @submit.prevent="hanldeSubmitForm" class="gap-[10px] items-end w-full">
                            ...
                            <div class="w-full flex justify-end">
                                <button :disabled="form.processing" class="me-[3px] mt-[10px] p-[3px] ps-[10px] pe-[10px] border rounded bg-[#BE202F] text-white font-bold text-[12px]">Cập nhật</button>
                            </div>
                        </form>
                    </div>
                </div>
...
const hanldeSubmitForm = () => {
    form.patch(route('admin.permission.update', permission.id), {
        onError: (xhr) => {
            console.log(xhr);
